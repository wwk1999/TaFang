...
{
    if (元始Source != null) 元始Source.Stop();
}
   ...
   {
       if (首领AudioSource != null) 首领AudioSource.volume = PlayerData.S.音效音量;
       if (音效Source != null) 音效Source.volume = PlayerData.S.音效音量;
       if (元始Source != null) 元始Source.volume = PlayerData.S.音效音量;
       foreach (var item in 人物AudioSource)
       {
           if (item != null) item.volume = PlayerData.S.音效音量;
       }
   ...
   {
      // 空保护:AudioController 可能还没初始化完就被事件触发(如 StartWindow.OnEnable 时序更早)
      if (BgAudioSource == null || UIClip == null) return;
   ...
   {
      if (BgAudioSource == null || 战斗Clip == null) return;
