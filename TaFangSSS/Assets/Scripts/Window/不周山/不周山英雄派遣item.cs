...
public class 不周山英雄派遣item : MonoBehaviour
{
    ...
    {
        ObserverModuleManager.S.UnRegisterEvent("不周山英雄派遣Item刷新",不周山英雄派遣Item刷新);
    }
    private void Awake()
    {
        ObserverModuleManager.S.RegisterEvent("不周山英雄派遣Item刷新",不周山英雄派遣Item刷新);
        bg.onClick.AddListener(() =>
        {
            HeroWindowController.S.不周山英雄派遣Index = index;
            ObserverModuleManager.S.SendEvent("显示不周山英雄派遣弹窗");
        });
    }

    public void 不周山英雄派遣Item刷新(object[] obj)
    {
        SetItem();
    }
    ...
    public void SetItem()
    {
        mask.SetActive(PlayerData.S.不周山寻宝Dic[HeroWindowController.S.当前不周山层数].寻宝);
