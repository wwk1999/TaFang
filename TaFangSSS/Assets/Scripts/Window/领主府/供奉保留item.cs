...
public class 供奉保留item : MonoBehaviour
{
    public Image icon;
    ...
    private void Start()
    {
        删除按钮.onClick.AddListener(() =>
        {
            ...
            ObserverModuleManager.S.SendEvent("SendUIToast","删除供奉成功,已灭口");
        });
        同意按钮.onClick.AddListener(() =>
        {
            if (PlayerData.S.当前供奉列表.Count >= 道场Config.领主府配置[PlayerData.S.建筑等级Dic[建筑Type.领主府]].供奉个数)
            {
                ObserverModuleManager.S.SendEvent("SendUIToast","当前上场供奉数量已满");
                return;
            }
            PlayerData.S.供奉保留列表.Remove(供奉);
