...
public class 突破item : MonoBehaviour
{
    ...
    public void SetItem()
    {
        ...
        icon.sprite=ResourcesConfig.Get突破灵物(PlayerData.S.当前轮回境界, quality);
        灵物名.text = 灵物突破Config.突破灵物名Dic[PlayerData.S.当前轮回境界];
        灵物名.colorGradientPreset=ResourcesConfig.Get品质TMP(quality);
        功德count.text=PlayerData.S.格式化数字(JingJieConfig.突破材料Dic[PlayerData.S.当前轮回境界][(int)(quality-1)]);
    }
    ...
    public void Start()
    {
        突破Button.onClick.AddListener(() =>
        {
            ...
            if (PlayerData.S.PropListDic[PropType.功德] < JingJieConfig.突破材料Dic[PlayerData.S.当前轮回境界][(int)(quality - 1)])
            {
